...
import { FiX, FiShoppingCart, FiArrowLeft, FiSearch, FiStar, FiCheck, FiTruck, FiShield, FiRepeat, FiGrid, FiList, FiFilter, FiSmartphone, FiMonitor, FiTv, FiCamera, FiBattery, FiWifi, FiBluetooth, FiCpu, FiTablet, FiWatch, FiHome, FiChevronDown, FiChevronUp } from 'react-icons/fi'
import { samsungProducts as dummySamsungProducts, getSamsungFilterOptions } from '../dummyData/samsungProductsData'
...
const SamsungStore = () => {
    ...
    const fetchSamsungProducts = async () => {
      try {
        ...
        if (Array.isArray(productsData) && productsData.length > 0) {
          // Filter for Samsung products and transform backend products data
          const samsungBackendProducts = productsData
            .filter(product => product.brand && product.brand.toLowerCase() === 'samsung')
            .map(product => ({
              id: product._id || product.id || `backend-${Math.random().toString(36).substr(2, 9)}`,
              name: product.name,
              description: product.description,
              brand: product.brand,
              category: getSamsungCategory(product.category),
              productType: getSamsungProductType(product.category, product.name),
              price: product.price,
              ...
              series: extractSeriesFromName(product.name),
              connectivity: getConnectivityFromProduct(product),
              screenSize: getScreenSizeFromProduct(product),
              features: product.features || ['Smart Feature', 'High Performance'],
          ...
          samsungBackendProducts.sort((a, b) => {
          ...
          const uniqueDummyProducts = dummySamsungProducts.map(product => ({
            ...product,
            id: `dummy-${product.id}`,
            source: 'dummy',
          ...
          // Combine both arrays
          const allProducts = [...samsungBackendProducts, ...uniqueDummyProducts];
          
          setBackendProducts(samsungBackendProducts);
          setProducts(allProducts);
          setUseFallbackData(false);
        } else {
          // No products from backend, use only dummy data
          console.log('No Samsung products from backend, using dummy data only');
          const uniqueDummyProducts = dummySamsungProducts.map(product => ({
            ...product,
            id: `dummy-${product.id}`,
            source: 'dummy',
        ...
        setBackendProducts([]);
        setUseFallbackData(true);
        setError('Unable to load Samsung products from server. Showing sample products.');
      } finally {
        setLoading(false);
      }
    };
  ...
  // Helper functions for Samsung products
  const getSamsungCategory = (backendCategory) => {
    if (!backendCategory) return 'Smartphones';
    ...
    return categoryMap[backendCategory] || backendCategory || 'Smartphones';
  };

  const getSamsungProductType = (category, name) => {
    if (!category) return 'Smartphone';
    
    const nameLower = name.toLowerCase();
    
    // Check for specific product types in name
    if (nameLower.includes('galaxy s')) return 'Galaxy S Series';
    if (nameLower.includes('galaxy a')) return 'Galaxy A Series';
    if (nameLower.includes('galaxy z')) return 'Galaxy Z Series';
    if (nameLower.includes('galaxy watch')) return 'Galaxy Watch';
    if (nameLower.includes('galaxy buds')) return 'Galaxy Buds';
    if (nameLower.includes('neo qled')) return 'Neo QLED TV';
    if (nameLower.includes('qled')) return 'QLED TV';
    if (nameLower.includes('the frame')) return 'The Frame TV';
    if (nameLower.includes('bespoke')) return 'Bespoke';
  ...
  const extractSeriesFromName = (name) => {
    const nameLower = name.toLowerCase();
    if (nameLower.includes('galaxy s')) return 'Galaxy S Series';
    if (nameLower.includes('galaxy a')) return 'Galaxy A Series';
    if (nameLower.includes('galaxy z')) return 'Galaxy Z Series';
    if (nameLower.includes('galaxy note')) return 'Galaxy Note Series';
    if (nameLower.includes('neo qled')) return 'Neo QLED Series';
    if (nameLower.includes('qled')) return 'QLED Series';
    if (nameLower.includes('odyssey')) return 'Odyssey Series';
    return 'Standard Series';
  };

  const getConnectivityFromProduct = (product) => {
    if (product.category === 'Mobile Phones' || product.category === 'Smartphones') {
      return '5G';
    }
    if (product.category === 'Tablets' || product.category === 'Laptops') {
      return 'WiFi + 4G/5G';
    }
    if (product.category === 'Wearables' || product.category === 'Watch') {
      return 'Bluetooth 5.0';
    }
    return 'WiFi';
  };

  const getScreenSizeFromProduct = (product) => {
    const name = product.name.toLowerCase();
    
    // Extract screen size from name
    const sizeMatch = name.match(/(\d+)(?:\.\d+)?\s*(?:inch|"|\s)/);
    if (sizeMatch) {
    ...
    if (product.category === 'Televisions') {
      return '55 inch';
    }
  ...
  // Get filter options based on actual products
  const filterOptions = useMemo(() => {
    return getSamsungFilterOptions(products);
  }, [products]);
  ...
  const handleCompletePurchase = (orderData) => {
    console.log('Samsung purchase completed:', orderData);
  ...
  const filteredProducts = useMemo(() => {
    ...
    if (filters.screenSize) filtered = filtered.filter(p => p.screenSize === filters.screenSize);
    ...
    if (filters.priceRange) {
      switch (filters.priceRange) {
        ...
        case '₹2,00,000 - ₹3,00,000': filtered = filtered.filter(p => p.price >= 200000 && p.price <= 300000); break;
        case '₹3,00,000 - ₹5,00,000': filtered = filtered.filter(p => p.price >= 300000 && p.price <= 500000); break;
  ...
  const categoryCounts = useMemo(() => {
    ...
    products.forEach(product => {
      if (counts[product.category] !== undefined) {
        ...
        // For any new categories from backend
      ...
      <div className="min-h-screen bg-linear-to-br from-gray-50 to-white flex flex-col">
        ...
        <div className="flex-1 flex items-center justify-center">
          <div className="text-center">
            ...
            <p className="text-gray-600">Loading Samsung products...</p>
          </div>
        </div>
      </div>
    );
    ...
    return (
      <div className="min-h-screen bg-linear-to-br from-gray-50 to-white">
        ...
        >
          ...
          <span className="font-medium">Back to Samsung Store</span>
        </button>
        
        <button
          onClick={() => setIsCartOpen(!isCartOpen)}
          className="fixed bottom-20 right-4 sm:bottom-6 sm:right-6 z-100 bg-blue-500 hover:bg-blue-600 text-white p-3 sm:p-4 rounded-full shadow-lg transition-all duration-300 transform hover:scale-110 focus:outline-none focus:ring-4 focus:ring-blue-300"
          aria-label="Open Cart"
        >
          <FiShoppingCart className="w-5 h-5 sm:w-6 sm:h-6" />
          {cartItemCount > 0 && (
            <span className="absolute -top-1 -right-1 sm:-top-2 sm:-right-2 bg-blue-700 text-white text-xs rounded-full h-5 w-5 sm:h-6 sm:w-6 flex items-center justify-center animate-pulse">
  ...
  if (checkoutMode) {
    ...
    if (checkoutType === 'single' && checkoutProduct) {
      productsForCheckout = [{
        ...
        brand: checkoutProduct.brand || 'Samsung',
        category: checkoutProduct.category || '',
        series: checkoutProduct.series || '',
        screenSize: checkoutProduct.screenSize || ''
      }];
    } else {
  ...
  return (
    <div className="min-h-screen bg-linear-to-br from-gray-50 to-white flex flex-col">
      ...
      <button
        ...
        className="fixed bottom-20 left-4 sm:hidden z-100 bg-blue-500 hover:bg-blue-600 text-white p-3 rounded-full shadow-lg transition-all duration-300 transform hover:scale-110 focus:outline-none focus:ring-4 focus:ring-blue-300 flex items-center justify-center"
              ...
              <div className="flex-1 overflow-y-auto p-3 sm:p-4 md:p-6">
                {cart.length === 0 ? (
                  <div className="text-center py-8 sm:py-12">
                    ...
                    <p className="text-gray-400 text-sm sm:text-base">Add Samsung products to get started</p>
                  </div>
                ) : (
                          ...
                          <div className="flex justify-between items-start">
                            <div className="flex-1 min-w-0">
                              ...
                              <p className="text-xs text-gray-500">
                                {item.colour && `${item.colour} • `}{item.size || item.storage || item.screenSize || ''}
                              </p>
                            </div>
                            <button
              ...
              {cart.length > 0 && (
                <div className="border-t p-4 sm:p-5 md:p-6">
                  ...
                  <button
                    ...
                    className="w-full bg-blue-500 hover:bg-blue-600 text-white py-3 sm:py-4 rounded-lg font-semibold transition-colors shadow-lg hover:shadow-xl text-sm sm:text-base"
                  >
      ...
      <main className="flex-1 relative z-0">
        <div className="min-h-screen bg-linear-to-b from-transparent to-gray-50">
          {/* Hero Section */}
          <div className="bg-linear-to-r from-blue-500 via-blue-600 to-blue-700 text-white shadow-lg">
              ...
              <div className="flex flex-col md:flex-row items-center justify-between gap-4">
                <div className="animate-fade-in mb-6 md:mb-0 md:w-2/3">
                  <div className="flex items-center gap-3 mb-4 sm:mb-6">
                    ...
                    <div>
                      <h1 className="text-2xl sm:text-3xl md:text-4xl lg:text-5xl font-bold mb-1 sm:mb-2">Samsung Store</h1>
                      <p className="text-sm sm:text-base md:text-lg opacity-90">Do What You Can't</p>
                    </div>
                  </div>
                  <p className="text-sm sm:text-base md:text-lg opacity-90 max-w-3xl mb-4 sm:mb-6 md:mb-8">
                    Discover the latest innovations in technology with Samsung. From cutting-edge smartphones 
                    and immersive TVs to smart home appliances and premium wearables, experience the future today.
                  </p>
                  <div className="flex flex-wrap gap-2 sm:gap-3">
                    <div className="bg-white bg-opacity-20 px-3 sm:px-4 py-1 sm:py-2 rounded-full backdrop-blur-sm">
                      <span className="font-medium text-sky-700 text-xs sm:text-sm">📱 Galaxy Ecosystem</span>
                    </div>
                    <div className="bg-white bg-opacity-20 px-3 sm:px-4 py-1 sm:py-2 rounded-full backdrop-blur-sm">
                      <span className="font-medium text-gray-800 text-xs sm:text-sm">📺 Neo QLED TVs</span>
                    </div>
                    <div className="bg-white bg-opacity-20 px-3 sm:px-4 py-1 sm:py-2 rounded-full backdrop-blur-sm">
                      <span className="font-medium text-green-800 text-xs sm:text-sm">🏠 Smart Home</span>
                    </div>
                    <div className="bg-white bg-opacity-20 px-3 sm:px-4 py-1 sm:py-2 rounded-full backdrop-blur-sm">
                      <span className="font-medium text-purple-800 text-xs sm:text-sm">⌚️ Galaxy Watch</span>
                    </div>
                  </div>
                </div>
            ...
            <div className="max-w-7xl mx-auto px-3 sm:px-4">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-6">
                {[
                  ...
                  { category: 'Home Appliances', icon: FiHome, count: categoryCounts['Home Appliances'] },
                  ...
                  { category: 'Tablets', icon: FiTablet, count: categoryCounts.Tablets },
                  { category: 'Audio', icon: FiCamera, count: categoryCounts.Audio },
                ].map(({ category, icon: Icon, count }) => (
                  <div 
                    key={category}
                    className={`relative overflow-hidden rounded-xl shadow-lg cursor-pointer transform transition-all duration-300 hover:scale-105 ${filters.category === category ? 'ring-4 ring-blue-500' : ''}`}
                    onClick={() => handleFilterChange('category', filters.category === category ? '' : category)}
                  >
                    <div className="h-40 sm:h-48 bg-linear-to-br from-blue-500 to-blue-700 flex items-center justify-center">
                ...
                <div className="flex items-center justify-between gap-4">
                  <div className="relative flex-1 max-w-xl">
                    <div className="relative">
                      ...
                      <input
                        type="text"
                        placeholder="Search Samsung products by name, category, or features..."
                        value={searchQuery}
                        onChange={(e) => setSearchQuery(e.target.value)}
                        className="w-full pl-12 pr-10 py-2.5 sm:py-3 bg-gray-50 border border-gray-200 rounded-full focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent text-sm sm:text-base"
                      />
                      {searchQuery && (
                        <button
                          onClick={() => setSearchQuery('')}
                  ...
                  <div className="flex items-center gap-3 sm:gap-4">
                    <div className="flex items-center gap-2">
                      ...
                      <button
                        ...
                        className={`p-2 rounded-lg ${viewMode === 'list' ? 'bg-blue-500 text-white' : 'bg-gray-100 text-gray-600 hover:bg-gray-200'}`}
                      >
          ...
          <div className="max-w-7xl mx-auto px-3 sm:px-4 py-6 sm:py-8">
            <div className="mb-4 sm:mb-6 md:mb-8">
              <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
                <div>
                  <h2 className="text-xl sm:text-2xl font-bold text-gray-900">Samsung Innovation Collection</h2>
                  <p className="text-gray-600 text-sm sm:text-base mt-1">
                    Experience cutting-edge technology that transforms your daily life
                    {error && (
                ...
                <div className="flex items-center gap-4 sm:gap-6">
                  ...
                  <div className="hidden sm:flex items-center gap-4 sm:gap-6">
                    <div className="flex items-center gap-2">
                      ...
                      <span className="text-sm text-gray-600">Latest Tech</span>
                    </div>
                    <div className="flex items-center gap-2">
                            ...
                            <div className="flex flex-wrap items-center gap-2 sm:gap-4 mb-3 sm:mb-4">
                              ...
                              {product.screenSize !== 'N/A' && (
                                <>
                                  ...
                                  <span className="text-gray-500 text-sm">Size: {product.screenSize}</span>
                                </>
                              )}
                              <span className="text-gray-500 hidden sm:inline">•</span>
                          ...
                          <div className="md:w-1/3 md:pl-4 lg:pl-6">
                            ...
                            <div className="flex gap-2 sm:gap-3">
                              ...
                              <button
                                ...
                                className="flex-1 bg-blue-500 hover:bg-blue-600 text-white px-3 sm:px-4 py-2 sm:py-3 rounded-lg font-medium transition-colors duration-200 text-sm sm:text-base"
                              >
                                Buy Now
                              </button>
                            </div>
            ...
            {filteredProducts.length === 0 && (
              <div className="text-center py-12 sm:py-16">
                ...
                <h3 className="text-lg sm:text-xl md:text-2xl font-bold text-gray-900 mb-2 sm:mb-3">No Samsung Products Found</h3>
                <p className="text-gray-600 text-sm sm:text-base mb-6 max-w-md mx-auto px-4">
                  Try adjusting your filters or search query to discover our Samsung innovation collection.
                </p>
                <button
                  onClick={clearAllFilters}
                  className="bg-linear-to-r from-blue-500 to-blue-700 text-white px-6 sm:px-8 py-2.5 sm:py-3 rounded-full font-semibold hover:from-blue-600 hover:to-blue-800 transition-all duration-300 transform hover:scale-105 shadow-lg text-sm sm:text-base"
                >
                  View All Products
                </button>
              </div>
          ...
          {/* Samsung Series Section */}
          <div className="bg-linear-to-r from-blue-600 to-blue-800 text-white py-12 sm:py-16">
            <div className="max-w-7xl mx-auto px-3 sm:px-4">
              <h2 className="text-2xl sm:text-3xl font-bold text-center mb-8 sm:mb-12">Samsung Ecosystem</h2>
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6 sm:gap-8">
                {[
                  { icon: FiSmartphone, title: 'Galaxy S Series', desc: 'Flagship smartphones with cutting-edge camera and performance' },
                  { icon: FiTv, title: 'Neo QLED 8K', desc: 'Revolutionary TVs with Quantum Mini LED technology' },
                  { icon: FiWatch, title: 'Galaxy Watch', desc: 'Advanced health monitoring with BioActive sensors' },
                  { icon: FiHome, title: 'Bespoke Home', desc: 'Customizable home appliances that match your style' },
                ].map((item, index) => (
          ...
          <div className="bg-white py-12 sm:py-16">
            <div className="max-w-7xl mx-auto px-3 sm:px-4">
              <h2 className="text-2xl sm:text-3xl font-bold text-center text-gray-900 mb-8 sm:mb-12">Why Choose Samsung?</h2>
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6 sm:gap-8">
                {[
                  { icon: FiCheck, title: 'Official Samsung Warranty', desc: 'Genuine Samsung products with official manufacturer warranty' },
                  { icon: FiTruck, title: 'Fast & Free Delivery', desc: 'Free shipping on all orders with installation services available' },
                  { icon: FiShield, title: 'Secure Shopping', desc: '100% secure payment gateway with data encryption protection' },
                  { icon: FiRepeat, title: 'Easy Returns', desc: '15-day return policy with easy pickup from your home' },
                ].map((item, index) => (
          ...
          {/* Samsung Technology Guide */}
          <div className="bg-linear-to-r from-blue-50 to-gray-50 py-12 sm:py-16">
            <div className="max-w-7xl mx-auto px-3 sm:px-4">
              <div className="bg-white rounded-2xl shadow-xl p-4 sm:p-6 md:p-8">
                <h2 className="text-xl sm:text-2xl font-bold text-center text-gray-900 mb-6 sm:mb-8">Samsung Technology Innovations</h2>
                <div className="grid grid-cols-1 md:grid-cols-3 gap-6 sm:gap-8">
                  <div>
                    <h3 className="font-bold text-base sm:text-lg text-gray-900 mb-3 sm:mb-4">Display Technology</h3>
                    <ul className="space-y-1.5 sm:space-y-2">
                      <li className="flex items-start">
                        <FiCheck className="w-4 h-4 sm:w-5 sm:h-5 text-green-500 mr-2 mt-0.5 shrink-0" />
                        <span className="text-gray-700 text-sm sm:text-base">Dynamic AMOLED 2X: Vibrant, eye-comfort display</span>
                      </li>
                      <li className="flex items-start">
                        <FiCheck className="w-4 h-4 sm:w-5 sm:h-5 text-green-500 mr-2 mt-0.5 shrink-0" />
                        <span className="text-gray-700 text-sm sm:text-base">Quantum Mini LED: Ultra-precise contrast control</span>
                      </li>
                      <li className="flex items-start">
                        <FiCheck className="w-4 h-4 sm:w-5 sm:h-5 text-green-500 mr-2 mt-0.5 shrink-0" />
                        <span className="text-gray-700 text-sm sm:text-base">Vision Booster: Enhanced outdoor visibility</span>
                      </li>
                      <li className="flex items-start">
                        <FiCheck className="w-4 h-4 sm:w-5 sm:h-5 text-green-500 mr-2 mt-0.5 shrink-0" />
                        <span className="text-gray-700 text-sm sm:text-base">Adaptive Refresh Rate: 1-120Hz for optimal battery</span>
                      </li>
                    </ul>
                  </div>
                  <div>
                    <h3 className="font-bold text-base sm:text-lg text-gray-900 mb-3 sm:mb-4">Camera Innovation</h3>
                    <ul className="space-y-1.5 sm:space-y-2">
                      <li className="flex items-start">
                        <FiCheck className="w-4 h-4 sm:w-5 sm:h-5 text-green-500 mr-2 mt-0.5 shrink-0" />
                        <span className="text-gray-700 text-sm sm:text-base">Adaptive Pixel Sensor: Brilliant photos in any light</span>
                      </li>
                      <li className="flex items-start">
                        <FiCheck className="w-4 h-4 sm:w-5 sm:h-5 text-green-500 mr-2 mt-0.5 shrink-0" />
                        <span className="text-gray-700 text-sm sm:text-base">Nightography: Stunning low-light photography</span>
                      </li>
                      <li className="flex items-start">
                        <FiCheck className="w-4 h-4 sm:w-5 sm:h-5 text-green-500 mr-2 mt-0.5 shrink-0" />
                        <span className="text-gray-700 text-sm sm:text-base">Expert RAW: Professional-grade manual controls</span>
                      </li>
                      <li className="flex items-start">
                        <FiCheck className="w-4 h-4 sm:w-5 sm:h-5 text-green-500 mr-2 mt-0.5 shrink-0" />
                        <span className="text-gray-700 text-sm sm:text-base">Director's View: Multi-camera recording</span>
                      </li>
                    </ul>
                  </div>
                  <div>
                    <h3 className="font-bold text-base sm:text-lg text-gray-900 mb-3 sm:mb-4">Smart Ecosystem</h3>
                    <ul className="space-y-1.5 sm:space-y-2">
                      <li className="flex items-start">
                        <FiCheck className="w-4 h-4 sm:w-5 sm:h-5 text-green-500 mr-2 mt-0.5 shrink-0" />
                        <span className="text-gray-700 text-sm sm:text-base">SmartThings: Unified control for all devices</span>
                      </li>
                      <li className="flex items-start">
                        <FiCheck className="w-4 h-4 sm:w-5 sm:h-5 text-green-500 mr-2 mt-0.5 shrink-0" />
                        <span className="text-gray-700 text-sm sm:text-base">Quick Share: Instant file sharing between devices</span>
                      </li>
                      <li className="flex items-start">
                        <FiCheck className="w-4 h-4 sm:w-5 sm:h-5 text-green-500 mr-2 mt-0.5 shrink-0" />
                        <span className="text-gray-700 text-sm sm:text-base">Multi Control: Use phone as trackpad for tablet/PC</span>
                      </li>
                      <li className="flex items-start">
                        <FiCheck className="w-4 h-4 sm:w-5 sm:h-5 text-green-500 mr-2 mt-0.5 shrink-0" />
                        <span className="text-gray-700 text-sm sm:text-base">Bixby Routines: Automated smart actions</span>
                      </li>
                    </ul>
                  </div>
                </div>
