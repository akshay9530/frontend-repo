...
import { calvinProducts, getCalvinFilterOptions } from '../dummyData'
import { FiX, FiShoppingCart, FiArrowLeft, FiSearch, FiStar, FiCheck, FiTruck, FiShield, FiRepeat, FiGrid, FiList, FiFilter, FiUser, FiHeart, FiTag, FiWatch, FiBriefcase, FiSun, FiMoon, FiPackage, FiGift, FiBox, FiChevronDown, FiChevronUp } from 'react-icons/fi'
...
const CalvinStore = () => {
  ...
  useEffect(() => {
    const fetchAllProducts = async () => {
      try {
        ...
        console.log('Backend response (all products) for Calvin Store:', data);
        ...
        if (Array.isArray(productsData) && productsData.length > 0) {
          // Filter for Calvin Klein brand or transform all products
          const calvinBackendProducts = productsData
            .filter(product => 
              ...
              product.category?.includes('Fragrances')
            )
            .map(product => ({
              id: product._id || product.id || `backend-${Math.random().toString(36).substr(2, 9)}`,
              name: product.name || 'Calvin Klein Product',
              description: product.description || 'Premium product from Calvin Klein collection',
              brand: product.brand || 'Calvin Klein',
              category: getCalvinCategoryFromBackend(product.category),
              productType: getCalvinProductTypeFromCategory(product.category),
              collection: getCalvinCollectionFromProduct(product),
              gender: getCalvinGenderFromProduct(product),
              price: product.price || 0,
              originalPrice: product.originalPrice || product.price || 0,
              discount: product.discount || 0,
              rating: product.rating || 4.5,
              reviews: product.reviews || Math.floor(Math.random() * 100),
              image: product.images && product.images[0] ? product.images[0].url : getCalvinDefaultImage(product.category),
              images: product.images ? product.images.map(img => img.url) : [getCalvinDefaultImage(product.category)],
              size: getCalvinSizeFromProduct(product),
              sizes: product.sizes || getDefaultSizesForCategory(product.category),
              colour: product.colors && product.colors.length > 0 ? product.colors[0] : getCalvinDefaultColor(),
              colors: product.colors || [getCalvinDefaultColor()],
              stock: product.stock || 10,
              isNew: product.isNewArrival || false,
              isFeatured: product.isFeatured || false,
              isOnSale: product.isOnSale || false,
              tags: product.tags || ['Calvin Klein', 'Premium'],
              createdAt: product.createdAt || new Date(),
              source: 'backend'
            }));
          
          // Sort backend products by creation date (newest first)
          calvinBackendProducts.sort((a, b) => {
          ...
          const uniqueDummyProducts = calvinProducts.map(product => ({
            ...product,
            id: `dummy-${product.id}`,
            source: 'dummy',
          ...
          // Combine both arrays
          const allProducts = [...calvinBackendProducts, ...uniqueDummyProducts];
          
          setBackendProducts(calvinBackendProducts);
        ...
        console.error('Error fetching Calvin products:', error);
  ...
  // Helper functions for Calvin Klein products
  const getCalvinCategoryFromBackend = (backendCategory) => {
  ...
  const getCalvinProductTypeFromCategory = (category) => {
    if (!category) return 'T-Shirt';
    
    const typeMap = {
      'Men Clothes': 'T-Shirt',
      'Women Clothes': 'Dress',
      'Men Watches': 'Wrist Watch',
      'Women Watches': 'Women\'s Watch',
      'Men Fragrances': 'Cologne',
      'Women Fragrances': 'Perfume',
      'Clothing': 'T-Shirt',
      'Watches': 'Wrist Watch',
      'Underwear': 'Briefs',
      'Lingerie': 'Bra',
      ...
      'Home': 'Home Fragrance',
      'Jewelry': 'Necklace',
      'Sportswear': 'Sports Bra'
    };
    
    return typeMap[category] || 'Product';
  };

  const getCalvinCollectionFromProduct = (product) => {
    if (product.collection) return product.collection;
    
    // Generate collection based on category or name
    const collections = ['Modern', 'Classic', 'Professional', 'Comfort', 'Signature', 'Minimalist', 'Luxury'];
    return collections[Math.floor(Math.random() * collections.length)];
  };

  const getCalvinGenderFromProduct = (product) => {
  ...
  const getCalvinDefaultImage = (category) => {
    const imageMap = {
      'Clothing': 'https://images.unsplash.com/photo-1523381210434-271e8be1f52b?ixlib=rb-4.0.3&auto=format&fit=crop&w=500&q=80',
      'Watches': 'https://images.unsplash.com/photo-1523170335258-f5ed11844a49?ixlib=rb-4.0.3&auto=format&fit=crop&w=500&q=80',
      'Underwear': 'https://images.unsplash.com/photo-1596755094514-f87e34085b2c?ixlib=rb-4.0.3&auto=format&fit=crop&w=500&q=80',
      'Fragrance': 'https://images.unsplash.com/photo-1541643600914-78b084683601?ixlib=rb-4.0.3&auto=format&fit=crop&w=500&q=80',
      'Accessories': 'https://images.unsplash.com/photo-1511499767150-a48a237f0083?ixlib=rb-4.0.3&auto=format&fit=crop&w=500&q=80'
    };
    
    return imageMap[category] || 'https://images.unsplash.com/photo-1523381210434-271e8be1f52b?ixlib=rb-4.0.3&auto=format&fit=crop&w=500&q=80';
  };

  const getCalvinSizeFromProduct = (product) => {
    if (product.size) return product.size;
    
    const sizes = ['S', 'M', 'L', 'XL', '36', '38', '40', '42', '44', '46', '100ml', '50ml', '200ml'];
    return sizes[Math.floor(Math.random() * sizes.length)];
  };

  const getDefaultSizesForCategory = (category) => {
    if (category?.includes('Clothing') || category?.includes('Underwear') || category?.includes('Lingerie')) {
      return ['S', 'M', 'L', 'XL', 'XXL'];
    } else if (category?.includes('Watches')) {
      return ['36mm', '38mm', '40mm', '42mm', '44mm'];
    } else if (category?.includes('Fragrance')) {
  ...
  const getCalvinDefaultColor = () => {
    const colors = ['Black', 'White', 'Gray', 'Navy', 'Red', 'Blue', 'Brown', 'Beige'];
  ...
  const filterOptions = useMemo(() => {
    ...
    let sizeOptions = [];
    if (filters.category === 'Clothing' || filters.category === 'Underwear' || filters.category === 'Lingerie') {
      const allSizes = currentProducts
        .filter(p => p.category === filters.category)
        .flatMap(p => p.sizes || [])
        .filter(Boolean);
      sizeOptions = [...new Set(allSizes)];
    } else if (filters.category === 'Watches') {
      sizeOptions = ['32mm', '36mm', '38mm', '40mm', '42mm', '44mm', '46mm'];
    } else if (filters.category === 'Fragrance') {
      sizeOptions = ['30ml', '50ml', '100ml', '200ml'];
    } else {
    ...
    // If no colors from products, use default Calvin colors
    if (colourOptions.length === 0) {
      colourOptions = ['Black', 'White', 'Gray', 'Navy', 'Red', 'Blue', 'Brown', 'Beige', 'Burgundy', 'Green'];
    }
    ...
    return {
      ...
      rating: ['4.0 and above', '4.2 and above', '4.5 and above', '4.7 and above']
    };
  ...
  const filteredProducts = useMemo(() => {
    ...
    if (filters.priceRange) {
      switch (filters.priceRange) {
        case 'Under ₹3,000': filtered = filtered.filter(p => p.price < 3000); break;
        ...
        case '₹18,000 - ₹25,000': filtered = filtered.filter(p => p.price >= 18000 && p.price <= 25000); break;
        case 'Above ₹25,000': filtered = filtered.filter(p => p.price > 25000); break;
        default: break;
      }
    }
  ...
  const categoryCounts = useMemo(() => {
    const counts = { 
      Clothing: 0, Underwear: 0, Lingerie: 0, Accessories: 0, 
      Watches: 0, Bags: 0, Shoes: 0, Fragrance: 0, 
      Home: 0, Jewelry: 0, Sportswear: 0 
    };
  ...
  return (
    <div className="min-h-screen bg-linear-to-br from-gray-50 to-white flex flex-col">
              ...
              <div className="flex-1 overflow-y-auto p-3 sm:p-4 md:p-6">
                {cart.length === 0 ? (
                  <div className="text-center py-8 sm:py-12">
                    ...
                    <p className="text-gray-400 text-sm sm:text-base">Add premium products to get started</p>
                  </div>
                ) : (
      ...
      {showMobileFilters && (
        <>
          <div 
            className="fixed inset-0 bg-black bg-opacity-50 backdrop-blur-sm z-30 sm:hidden"
            onClick={() => {
              setShowMobileFilters(false);
              setMobileActiveDropdown(null);
            }}
        ...
        <div className="min-h-screen bg-linear-to-b from-transparent to-gray-50">
              ...
              <div className="flex flex-col md:flex-row items-center justify-between">
                <div className="animate-fade-in mb-6 md:mb-0 md:w-2/3">
                  <div className="flex items-center gap-3 mb-4 sm:mb-6">
                    <div className="w-16 h-16 sm:w-20 sm:h-20 md:w-24 md:h-24 bg-white rounded-full flex items-center justify-center shadow-2xl">
                      <img 
                        src="https://images.seeklogo.com/logo-png/2/1/calvin-klein-logo-png_seeklogo-25074.png" 
                        alt="Logo" 
                        className="w-14 h-14 sm:w-18 sm:h-18 md:w-20 md:h-20 rounded-3xl object-contain"
                      />
                    </div>
                    <div>
                      <h1 className="text-2xl sm:text-3xl md:text-4xl lg:text-5xl font-bold mb-1 sm:mb-2">Premium Store</h1>
                      <p className="text-sm sm:text-base md:text-lg opacity-90">
                        {useFallbackData ? 'Sample Calvin Klein Collection' : 'Calvin Klein Premium Collection'}
                      </p>
                      {error && (
                  ...
                  <p className="text-sm sm:text-base md:text-lg opacity-90 max-w-3xl mb-4 sm:mb-6 md:mb-8">
                    Discover premium fashion, accessories, and fragrances. 
                    From timeless essentials to contemporary designs, experience minimalist luxury 
                    that defines modern style.
                  </p>
                  <div className="flex flex-wrap gap-2 sm:gap-3 md:gap-4">
                    <div className="bg-white bg-opacity-20 px-3 py-1 sm:px-4 sm:py-2 rounded-full backdrop-blur-sm text-xs sm:text-sm">
                      <span className="font-medium text-stone-800">👔 Premium Apparel</span>
                    </div>
                    <div className="bg-white bg-opacity-20 px-3 py-1 sm:px-4 sm:py-2 rounded-full backdrop-blur-sm text-xs sm:text-sm">
                      <span className="font-medium text-purple-600">⌚ Luxury Watches</span>
                    </div>
                    <div className="bg-white bg-opacity-20 px-3 py-1 sm:px-4 sm:py-2 rounded-full backdrop-blur-sm text-xs sm:text-sm">
                      <span className="font-medium text-cyan-950">⚜️ New Styles</span>
                    </div>
                    <div className="bg-white bg-opacity-20 px-3 py-1 sm:px-4 sm:py-2 rounded-full backdrop-blur-sm text-xs sm:text-sm">
                      <span className="font-medium text-pink-700">🧴 Iconic Fragrances</span>
                    </div>
                  </div>
                </div>
          ...
          <div className="bg-white py-6 sm:py-8 border-b border-gray-200">
            <div className="max-w-7xl mx-auto px-3 sm:px-4">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-6">
                <div 
                  className={`relative overflow-hidden rounded-xl shadow-lg cursor-pointer transform transition-all duration-300 hover:scale-105 ${filters.category === 'Clothing' ? 'ring-2 sm:ring-4 ring-black' : ''}`}
                ...
                >
                  <div className="h-40 sm:h-48 bg-linear-to-br from-gray-900 to-black flex items-center justify-center">
                    <div className="text-center text-white p-4 sm:p-6">
                      ...
                      <p className="text-gray-200 mt-1 text-sm sm:text-base">{categoryCounts.Clothing || 0} Styles</p>
                    </div>
                  </div>
                </div>
                <div 
                ...
                >
                  <div className="h-40 sm:h-48 bg-linear-to-br from-gray-800 to-gray-900 flex items-center justify-center">
                    <div className="text-center text-white p-4 sm:p-6">
                      ...
                      <p className="text-gray-200 mt-1 text-sm sm:text-base">{categoryCounts.Watches || 0} Styles</p>
                    </div>
                  </div>
                </div>
                <div 
                  className={`relative overflow-hidden rounded-xl shadow-lg cursor-pointer transform transition-all duration-300 hover:scale-105 ${filters.category === 'Underwear' || filters.category === 'Lingerie' ? 'ring-2 sm:ring-4 ring-black' : ''}`}
                  onClick={() => handleFilterChange('category', filters.category === 'Underwear' ? '' : 'Underwear')}
                >
                  <div className="h-40 sm:h-48 bg-linear-to-br from-gray-700 to-gray-800 flex items-center justify-center">
                    <div className="text-center text-white p-4 sm:p-6">
                      ...
                      <h3 className="text-lg sm:text-xl font-bold">Underwear & Lingerie</h3>
                      <p className="text-gray-200 mt-1 text-sm sm:text-base">
                        {(categoryCounts.Underwear || 0) + (categoryCounts.Lingerie || 0)} Styles
                      </p>
                    </div>
                  </div>
                </div>
          ...
          <div className="max-w-7xl mx-auto px-3 sm:px-4 py-6 sm:py-8">
            <div className="mb-6 sm:mb-8">
              <div className="flex items-center justify-between">
                <div>
                  <h2 className="text-lg sm:text-xl md:text-2xl font-bold text-gray-900">Premium Collection</h2>
            ...
            {filteredProducts.length > 0 ? (
              ...
              <div className="text-center py-12 sm:py-16">
                ...
                <p className="text-gray-600 mb-6 sm:mb-8 max-w-md mx-auto px-4 text-sm sm:text-base">
                  Try adjusting your filters or search query to discover our premium collection.
                </p>
                <button
                  onClick={clearAllFilters}
          ...
          <div className="bg-linear-to-r from-gray-900 to-black text-white py-12 sm:py-16">
            <div className="max-w-7xl mx-auto px-3 sm:px-4">
              <h2 className="text-xl sm:text-2xl md:text-3xl font-bold text-center mb-8 sm:mb-12">Premium Collections</h2>
              <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-6 sm:gap-8">
                <div className="text-center p-4">
                  <div className="w-16 h-16 sm:w-20 sm:h-20 bg-white rounded-full flex items-center justify-center mx-auto mb-4 sm:mb-6 transform hover:scale-110 transition-transform duration-300">
                    <FiBriefcase className="w-8 h-8 sm:w-10 sm:h-10 text-black" />
                  </div>
                  <h3 className="font-bold text-lg sm:text-xl mb-2 sm:mb-3">Professional</h3>
                  <p className="text-gray-200 text-sm sm:text-base">Tailored suits, dress shirts, and office essentials</p>
                </div>
                <div className="text-center p-4">
                  <div className="w-16 h-16 sm:w-20 sm:h-20 bg-white rounded-full flex items-center justify-center mx-auto mb-4 sm:mb-6 transform hover:scale-110 transition-transform duration-300">
                    <FiSun className="w-8 h-8 sm:w-10 sm:h-10 text-black" />
                  </div>
                  <h3 className="font-bold text-lg sm:text-xl mb-2 sm:mb-3">Modern Comfort</h3>
                  <p className="text-gray-200 text-sm sm:text-base">Comfort-focused underwear and loungewear</p>
                </div>
                <div className="text-center p-4">
                  <div className="w-16 h-16 sm:w-20 sm:h-20 bg-white rounded-full flex items-center justify-center mx-auto mb-4 sm:mb-6 transform hover:scale-110 transition-transform duration-300">
                    <FiWatch className="w-8 h-8 sm:w-10 sm:h-10 text-black" />
                  </div>
                  <h3 className="font-bold text-lg sm:text-xl mb-2 sm:mb-3">Timepieces</h3>
                  <p className="text-gray-200 text-sm sm:text-base">Minimalist watches and premium accessories</p>
                </div>
                <div className="text-center p-4">
                  <div className="w-16 h-16 sm:w-20 sm:h-20 bg-white rounded-full flex items-center justify-center mx-auto mb-4 sm:mb-6 transform hover:scale-110 transition-transform duration-300">
                    <FiBox className="w-8 h-8 sm:w-10 sm:h-10 text-black" />
                  </div>
                  <h3 className="font-bold text-lg sm:text-xl mb-2 sm:mb-3">Fragrances</h3>
                  <p className="text-gray-200 text-sm sm:text-base">Iconic scents and premium fragrance collections</p>
                </div>
              </div>
            </div>
          </div>
          ...
          <div className="bg-white py-12 sm:py-16">
            <div className="max-w-7xl mx-auto px-3 sm:px-4">
              <h2 className="text-xl sm:text-2xl md:text-3xl font-bold text-center text-gray-900 mb-8 sm:mb-12">Why Shop With Us?</h2>
              <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-6">
                <div className="text-center p-4 sm:p-6">
                  ...
                  <p className="text-gray-600 text-sm sm:text-base">100% genuine products with official warranty</p>
                </div>
                <div className="text-center p-4 sm:p-6">
                  <div className="w-12 h-12 sm:w-16 sm:h-16 bg-black rounded-full flex items-center justify-center mx-auto mb-4 sm:mb-6">
                    <FiTruck className="w-6 h-6 sm:w-8 sm:h-8 text-white" />
                  </div>
                  <h3 className="font-bold text-base sm:text-lg text-gray-900 mb-2 sm:mb-3">Premium Shipping</h3>
                  <p className="text-gray-600 text-sm sm:text-base">Free express delivery on orders above ₹5,000 across India</p>
                </div>
                <div className="text-center p-4 sm:p-6">
                  <div className="w-12 h-12 sm:w-16 sm:h-16 bg-black rounded-full flex items-center justify-center mx-auto mb-4 sm:mb-6">
                  ...
                  <p className="text-gray-600 text-sm sm:text-base">Premium quality assurance with satisfaction guarantee</p>
                </div>
                <div className="text-center p-4 sm:p-6">
                  <div className="w-12 h-12 sm:w-16 sm:h-16 bg-black rounded-full flex items-center justify-center mx-auto mb-4 sm:mb-6">
          ...
          <div className="bg-linear-to-r from-gray-50 to-gray-100 py-12 sm:py-16">
            <div className="max-w-7xl mx-auto px-3 sm:px-4">
              <div className="bg-white rounded-xl sm:rounded-2xl shadow-xl p-6 sm:p-8">
                <h2 className="text-lg sm:text-xl md:text-2xl font-bold text-center text-gray-900 mb-6 sm:mb-8">Care & Style Guide</h2>
                <div className="grid grid-cols-1 md:grid-cols-3 gap-6 sm:gap-8">
                  <div>
                    <h3 className="font-bold text-base sm:text-lg text-gray-900 mb-3 sm:mb-4">Premium Fabric Care</h3>
                    <ul className="space-y-2">
                      <li className="flex items-start">
                        <FiCheck className="w-4 h-4 sm:w-5 sm:h-5 text-green-500 mr-2 mt-0.5" />
                        <span className="text-gray-700 text-sm sm:text-base">Wool Suits: Dry clean only, store in garment bags</span>
                      </li>
                      <li className="flex items-start">
                        <FiCheck className="w-4 h-4 sm:w-5 sm:h-5 text-green-500 mr-2 mt-0.5" />
                        <span className="text-gray-700 text-sm sm:text-base">Cotton Dress Shirts: Machine wash cold, iron on medium</span>
                      </li>
                      <li className="flex items-start">
                        <FiCheck className="w-4 h-4 sm:w-5 sm:h-5 text-green-500 mr-2 mt-0.5" />
                        <span className="text-gray-700 text-sm sm:text-base">Leather Accessories: Clean with damp cloth, condition regularly</span>
                      </li>
                      <li className="flex items-start">
                        <FiCheck className="w-4 h-4 sm:w-5 sm:h-5 text-green-500 mr-2 mt-0.5" />
                        <span className="text-gray-700 text-sm sm:text-base">Watches: Avoid water contact, clean with soft cloth</span>
                      </li>
                    </ul>
                  </div>
                  <div>
                    <h3 className="font-bold text-base sm:text-lg text-gray-900 mb-3 sm:mb-4">Size & Fit Guide</h3>
                    <div className="space-y-3">
                      <div className="flex justify-between border-b pb-2">
                        <span className="text-gray-700 text-sm sm:text-base">Modern Fit</span>
                        <span className="text-gray-600 text-xs sm:text-sm">Contemporary silhouette with comfortable ease</span>
                      </div>
                      <div className="flex justify-between border-b pb-2">
                        <span className="text-gray-700 text-sm sm:text-base">Slim Fit</span>
                        <span className="text-gray-600 text-xs sm:text-sm">Close to body, tailored through chest and waist</span>
                      </div>
                      <div className="flex justify-between border-b pb-2">
                        <span className="text-gray-700 text-sm sm:text-base">Regular Fit</span>
                        <span className="text-gray-600 text-xs sm:text-sm">Classic cut with relaxed comfort throughout</span>
                      </div>
                      <div className="flex justify-between border-b pb-2">
                        <span className="text-gray-700 text-sm sm:text-base">Bodycon Fit</span>
                        <span className="text-gray-600 text-xs sm:text-sm">Form-fitting design that follows body contours</span>
                      </div>
                    </div>
                  </div>
                  <div>
                    <h3 className="font-bold text-base sm:text-lg text-gray-900 mb-3 sm:mb-4">Style Tips</h3>
                    <ul className="space-y-2">
                      <li className="flex items-start">
                        <FiCheck className="w-4 h-4 sm:w-5 sm:h-5 text-green-500 mr-2 mt-0.5" />
                        <span className="text-gray-700 text-sm sm:text-base">Layer minimalist pieces for sophisticated looks</span>
                      </li>
                      <li className="flex items-start">
                        <FiCheck className="w-4 h-4 sm:w-5 sm:h-5 text-green-500 mr-2 mt-0.5" />
                        <span className="text-gray-700 text-sm sm:text-base">Mix classic neutrals for timeless elegance</span>
                      </li>
                      <li className="flex items-start">
                        <FiCheck className="w-4 h-4 sm:w-5 sm:h-5 text-green-500 mr-2 mt-0.5" />
                        <span className="text-gray-700 text-sm sm:text-base">Accessorize with watches and belts</span>
                      </li>
                      <li className="flex items-start">
                        <FiCheck className="w-4 h-4 sm:w-5 sm:h-5 text-green-500 mr-2 mt-0.5" />
                        <span className="text-gray-700 text-sm sm:text-base">Complete your look with signature fragrances</span>
                      </li>
                    </ul>
                  </div>
                </div>
