...
import { menFragranceData as dummyFragrances } from '../dummyData/menFragranceData';
...
const MenFragrance = () => {
  ...
  useEffect(() => {
    const fetchMenFragrances = async () => {
      try {
        setLoading(true);
        ...
        if (Array.isArray(productsData) && productsData.length > 0) {
          // Filter for men's fragrances ONLY
          const menFragrancesBackend = productsData.filter(product => {
            ...
            const isMenCategory = 
              ...
              catLower === 'men fragrances' ||
              catLower === 'men fragrance' ||
              catLower === 'male fragrances';
            ...
            const isWomenCategory = 
              ...
              catLower === 'women fragrances' ||
              catLower === 'women fragrance' ||
              catLower === 'female fragrances';
          ...
          console.log('Filtered men fragrances from backend:', menFragrancesBackend.length);
          
          // Transform backend products data
          const transformedBackendProducts = menFragrancesBackend.map(product => ({
            id: product._id || product.id || `backend-${Math.random().toString(36).substr(2, 9)}`,
            name: product.name,
            description: product.description,
            brand: product.brand,
            category: 'Fragrance',
            productType: product.productType || 'Eau de Toilette',
            price: product.price,
            discount: product.discount || 0,
            rating: product.rating || 4.5,
            reviews: product.reviews || Math.floor(Math.random() * 100),
            image: product.images && product.images[0] ? product.images[0].url : 'https://images.unsplash.com/photo-1541643600914-78b084683601?w=900&q=80',
            images: product.images ? product.images.map(img => img.url) : ['https://images.unsplash.com/photo-1541643600914-78b084683601?w=900&q=80'],
            size: product.sizes && product.sizes.length > 0 ? product.sizes[0] : '100ml',
            sizes: product.sizes || ['100ml', '50ml', '200ml'],
            storage: '',
            colour: product.colors && product.colors.length > 0 ? product.colors[0] : 'Clear',
            colors: product.colors || ['Clear', 'Blue', 'Green', 'Gold', 'Brown', 'Silver', 'Amber', 'Black'],
          ...
          const uniqueDummyFragrances = dummyFragrances.map(product => ({
            ...product,
            id: `dummy-${product.id}`,
            source: 'dummy',
          ...
          // Combine both arrays
          const allProducts = [...transformedBackendProducts, ...uniqueDummyFragrances];
          ...
          setProducts(uniqueDummyFragrances);
          setBackendProducts([]);
  ...
  const getBrandType = (brand) => {
    ...
    // Men's fragrance brands
    const menBrands = [
      'axe', 'old spice', 'nivea men', 'l\'oreal men', 'loreal men',
      'gillette', 'brut', 'davidoff', 'hugo boss', 'boss',
      'paco rabanne', '1 million', 'jean paul gaultier', 
      'drakkar noir', 'aqua di gio', 'adidas', 'diesel',
      'dunhill', 'ferrari', 'kenzo', 'lacoste', 'montblanc',
      'armani', 'dior homme', 'dior', 'chanel', 'versace',
      'dolce & gabbana', 'dolce and gabbana', 'd&g', 'tom ford'
    ];
    
    // Women's fragrance brands
    const womenBrands = [
      'chanel', 'dior', 'lancôme', 'lancome', 'gucci', 'narciso', 
      'chloé', 'chloe', 'jo malone', 'jo malone london', 'ysl',
      'yves saint laurent', 'viktor&rolf', 'viktor and rolf',
      'tom ford', 'dolce & gabbana', 'dolce and gabbana', 'd&g',
      'versace', 'bvlgar', 'giorgio armani', 'armani', 
      'prada', 'hermes', 'burberry', 'calvin klein', 'ck',
      'marc jacobs', 'ralph lauren', 'estee lauder', 'elizabeth arden',
      'clinique', 'givenchy', 'guerlain', 'lancôme', 'lancome'
    ];
  ...
  // Helper function to map fragrance characteristics to categories
  const getFragranceCategory = (brand, description = '') => {
    // Brand-based category mapping
    const brandCategoryMap = {
      'Dior': 'Fresh',
      'Giorgio Armani': 'Fresh',
      'Versace': 'Fresh',
      'Paco Rabanne': 'Fresh',
      'Davidoff': 'Fresh',
      'Issey Miyake': 'Fresh',
      ...
      'Montblanc': 'Woody',
      'Hugo Boss': 'Woody',
      'Jean Paul Gaultier': 'Oriental',
      'Dolce & Gabbana': 'Oriental',
      'Tom Ford': 'Oriental',
      'Prada': 'Leather'
    };
    
    return brandCategoryMap[brand] || 'Fresh';
  };
  ...
  const filterOptions = useMemo(() => {
    ...
    if (filters.category) {
      const allColors = currentProducts
        .filter(p => getFragranceCategory(p.brand) === filters.category)
        .flatMap(p => p.colors || [])
        .filter(Boolean);
    ...
    if (colourOptions.length === 0) {
      colourOptions = ['Blue', 'Green', 'Gold', 'Brown', 'Silver', 'Clear', 'Amber', 'Black'];
    }
    
    return {
      sort: ['Newest', 'Price: Low to High', 'Price: High to Low', 'Popularity', 'Customer Rating'],
      brand: brands.sort(),
      category: ['All', 'Fresh', 'Woody', 'Aquatic', 'Spicy', 'Oriental', 'Leather'].sort(),
  ...
  const filteredFragrances = useMemo(() => {
    ...
    if (filters.category && filters.category !== 'All') {
      filtered = filtered.filter(p => {
        // Map fragrance characteristics to categories
        const categoryMap = {
          'Fresh': ['Dior', 'Giorgio Armani', 'Versace', 'Paco Rabanne', 'Davidoff', 'Issey Miyake'],
          'Woody': ['Chanel', 'Hermès', 'Burberry', 'Montblanc', 'Hugo Boss'],
          'Aquatic': ['Giorgio Armani', 'Paco Rabanne', 'Davidoff', 'Issey Miyake'],
          'Spicy': ['Dior', 'Paco Rabanne', 'Hugo Boss', 'Montblanc'],
          'Oriental': ['Jean Paul Gaultier', 'Dolce & Gabbana', 'Tom Ford'],
          'Leather': ['Dior', 'Montblanc', 'Prada']
        };
    ...
    if (filters.priceRange) {
      switch (filters.priceRange) {
        ...
        case '₹3000 - ₹6000': filtered = filtered.filter(p => p.price >= 3000 && p.price <= 6000); break;
        case '₹6000 - ₹9000': filtered = filtered.filter(p => p.price >= 6000 && p.price <= 9000); break;
        case '₹9000 - ₹12000': filtered = filtered.filter(p => p.price >= 9000 && p.price <= 12000); break;
        case 'Above ₹12000': filtered = filtered.filter(p => p.price > 12000); break;
        default: break;
      }
    }
        ...
        <div className="min-h-screen bg-linear-to-b from-transparent to-gray-50">
              ...
              <div className="flex flex-col sm:flex-row items-center justify-between gap-4">
                <div className="animate-fade-in text-center sm:text-left">
                  <h1 className="text-2xl sm:text-3xl md:text-4xl font-bold">
                    {useFallbackData ? 'Men\'s Luxury Fragrances' : 'Men\'s Fragrances Collection'}
                  </h1>
          ...
          <div className="max-w-7xl mx-auto px-3 sm:px-4 py-6 sm:py-8 md:py-12">
            ...
            <div className="mb-4 sm:mb-6 md:mb-8">
              <p className="text-gray-600 text-sm sm:text-base">
                Showing <span className="font-semibold">{filteredFragrances.length}</span> fragrance{filteredFragrances.length !== 1 ? 's' : ''}
              </p>
            </div>
            ...
            {filteredFragrances.length > 0 ? (
              ...
              <div className="text-center py-12 sm:py-16 md:py-20">
                ...
                <p className="text-gray-600 text-sm sm:text-base mb-6 max-w-md mx-auto px-4">
                  Try adjusting your filters to discover amazing scents.
                </p>
                <button
                  onClick={clearAllFilters}
          ...
          <div className="bg-linear-to-r from-blue-50 to-cyan-50 py-12 md:py-16 mt-12">
            <div className="max-w-7xl mx-auto px-3 sm:px-4">
              <h2 className="text-2xl md:text-3xl font-bold text-center text-gray-900 mb-8 md:mb-12">Why Choose Our Men's Fragrances?</h2>
              <div className="grid grid-cols-1 md:grid-cols-3 gap-6 md:gap-8">
                <div className="text-center">
                  ...
                  <p className="text-gray-600 text-sm md:text-base">Made with the finest ingredients for long-lasting fragrance</p>
                </div>
                <div className="text-center">
                  <div className="w-12 h-12 md:w-16 md:h-16 bg-blue-100 rounded-full flex items-center justify-center mx-auto mb-3 md:mb-4 transform hover:scale-110 transition-transform duration-300">
                    <svg className="w-6 h-6 md:w-8 md:h-8 text-blue-600" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  ...
                  <h3 className="font-semibold text-base md:text-lg text-gray-900 mb-1 md:mb-2">Long Lasting</h3>
                  <p className="text-gray-600 text-sm md:text-base">High concentration formulas that last all day</p>
                </div>
                <div className="text-center">
                  <div className="w-12 h-12 md:w-16 md:h-16 bg-blue-100 rounded-full flex items-center justify-center mx-auto mb-3 md:mb-4 transform hover:scale-110 transition-transform duration-300">
                    <svg className="w-6 h-6 md:w-8 md:h-8 text-blue-600" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  ...
                  <p className="text-gray-600 text-sm md:text-base">Free delivery on all orders over ₹999</p>
                </div>
              </div>
            </div>
          </div>
