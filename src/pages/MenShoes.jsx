...
import { menShoesData as dummyProducts } from '../dummyData/menShoesData'
...
const MenShoes = () => {
  ...
  const getBrandType = (brand) => {
    ...
    const menBrands = [
      ...
      'vans', 'new balance', 'skechers', 'clarks',
      'timberland', 'cat', 'dr. martens', 'dr martens',
      'ecco', 'rockport', 'cole haan', 'steve madden',
      'aldo', 'hush puppies', 'bata', 'woodland',
      'red tape', 'fila', 'asics', 'under armour',
      'lotto', 'sparx', 'li-ning', 'allen edmonds',
      'johnston & murphy', 'florsheim', 'bostonian',
      'geox', 'kenneth cole', 'tommy hilfiger', 'calvin klein',
      'hugo boss', 'lacoste', 'ralph lauren', 'polo',
      'levi\'s', 'wrangler', 'dickies', 'carhartt'
    ];
    
    // Women's shoe brands
    const womenBrands = [
      'aldo', 'steve madden', 'charles & keith', 'macy\'s',
      'nine west', 'naturalizer', 'easy spirit', 'sam & libby',
      'bandolino', 'anne klein', 'vince camuto', 'jessica simpson',
      'guess', 'michael kors', 'kate spade', 'tory burch',
      'jimmy choo', 'manolo blahnik', 'christian louboutin',
      'badgley mischka', 'betsey johnson', 'dolce vita',
      'franco sarto', 'chinese laundry', 'report', 'bcbg',
      'call it spring', 'spring', 'clarks', 'skechers',
      'naturalizer', 'comfortiva', 'aerosoles', 'sofft',
      'life stride', 'vionic', 'naot', 'mephisto',
      'birkenstock', 'crocs', 'ugg', 'bearpaw'
    ];
  ...
  const formatRating = (rating) => {
    ...
    if (isNaN(numRating)) return 4.0;
    // Cap rating at 5.0 and round to 1 decimal place
    return Math.min(5.0, Math.round(numRating * 10) / 10);
  };
    ...
    const fetchAllProducts = async () => {
      try {
        ...
        if (Array.isArray(productsData) && productsData.length > 0) {
          // Filter for men's shoes ONLY
          const mensShoesBackend = productsData.filter(product => {
            ...
            const isWomenCategory = 
              ...
              catLower.includes('ladies') ||
              catLower.includes('girl') ||
              catLower === 'women shoes' ||
              catLower === 'women sneakers' ||
              catLower === 'female shoes' ||
              catLower === 'women footwear';
            ...
            const isWomenTag = tags.some(tag => 
            ...
            // Also check if it's a shoe product
            const isShoe = 
              ...
              descLower.includes('shoe') ||
              descLower.includes('sneaker') ||
              descLower.includes('footwear') ||
              tags.some(tag => 
            ...
            // Return true if it's a men's shoe AND NOT a women's shoe
            return isShoe && (isMenCategory || isMenGender || isMenNameDesc || isMenTag || isMenBrand) && 
                   !(isWomenCategory || isWomenGender || isWomenNameDesc || isWomenTag || isWomenBrand);
          });
          
          console.log('Filtered men shoes from backend:', mensShoesBackend.length);
          
          // Transform backend products data
          const transformedBackendProducts = mensShoesBackend.map(product => {
            // Determine shoe type
            const name = product.name || '';
            ...
            if (name.toLowerCase().includes('formal') || desc.toLowerCase().includes('formal') || category.toLowerCase().includes('formal')) {
              shoeType = 'Formal';
            ...
            const sizes = product.sizes || ['US 8', 'US 9', 'US 10', 'US 11'];
            const colors = product.colors || ['Black', 'White', 'Brown', 'Blue'];
            ...
            return {
              ...
              category: shoeType,
              productType: product.model || shoeType,
              price: product.price,
          ...
          // Ensure dummy products have unique IDs and format ratings
          const uniqueDummyProducts = dummyProducts.map(product => ({
            ...product,
            id: `dummy-${product.id}`,
            source: 'dummy',
            rating: formatRating(product.rating), // Format rating here too
          ...
          console.log('No shoe products from backend, using dummy data only');
          const uniqueDummyProducts = dummyProducts.map(product => ({
            ...product,
            id: `dummy-${product.id}`,
            source: 'dummy',
        ...
        setBackendProducts([]);
        setUseFallbackData(true);
        setError('Unable to load shoes from server. Showing sample products.');
      } finally {
        setLoading(false);
      }
    };
  ...
  const filterOptions = useMemo(() => {
    ...
    const categories = ['All', 'Sneakers', 'Formal', 'Boots', 'Sports', 'Loafers', 'Sandals'];
  ...
  const addToCartWithDelay = async (product) => {
    ...
    const productForCart = {
      ...
      size: product.size || '',
      brand: product.brand || '',
      category: product.category || 'Men Shoes',
      quantity: 1,
  ...
  const filteredShoes = useMemo(() => {
    ...
    if (filters.priceRange) {
      switch (filters.priceRange) {
        case 'Under ₹3000': filtered = filtered.filter(p => p.price < 3000); break;
        ...
        case 'Above ₹15000': filtered = filtered.filter(p => p.price > 15000); break;
        default: break;
      }
    }
  ...
  if (productDetailMode && selectedProduct) {
    return (
      <div className="min-h-screen bg-linear-to-br from-gray-50 to-white">
        <button
          onClick={goBack}
          className="fixed top-4 left-4 z-50 flex items-center gap-2 bg-white hover:bg-gray-100 text-gray-800 px-3 sm:px-4 py-2 rounded-full shadow-lg transition-all duration-200 hover:shadow-xl focus:outline-none focus:ring-2 focus:ring-green-500 text-sm sm:text-base"
        >
  ...
  if (checkoutMode) {
    ...
    if (checkoutType === 'single' && checkoutProduct) {
      productsForCheckout = [{
        ...
        category: checkoutProduct.category || 'Men Shoes',
  ...
  return (
    <div className="min-h-screen bg-linear-to-br from-gray-50 to-white flex flex-col">
      ...
      <button
        ...
        className="fixed bottom-20 left-4 sm:hidden z-100 bg-green-600 hover:bg-green-700 text-white p-3 rounded-full shadow-lg transition-all duration-300 transform hover:scale-110 focus:outline-none focus:ring-4 focus:ring-green-300 flex items-center justify-center"
          ...
          <div className="fixed inset-0 z-40 sm:hidden flex">
            ...
            <div className="w-4/5 max-w-sm bg-white shadow-xl h-full overflow-y-auto">
              <div className="p-4 border-b flex items-center justify-between">
                <h2 className="text-lg font-bold">Shoe Filters</h2>
                <button 
                  onClick={() => {
                    setShowMobileFilters(false);
                    setMobileActiveDropdown(null);
              ...
              <div className="p-4 border-b">
                <div className="relative">
                  ...
                  <input
                    ...
                    className="w-full pl-9 pr-4 py-2 bg-gray-50 border border-gray-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-green-500 focus:border-transparent text-sm"
                  />
                  {searchQuery && (
                    <button
                          ...
                          filterOptions[key].map(option => (
                            <button
                              ...
                              className={`w-full text-left p-3 rounded-lg mb-1 last:mb-0 hover:bg-gray-50 transition-colors ${
                                filters[key] === option ? 'bg-green-50 text-green-600 border border-green-200' : 'border border-transparent'
                              }`}
                            >
                              {option}
                            </button>
      ...
      <main className="flex-1 relative z-0">
        <div className="min-h-screen bg-linear-to-b from-transparent to-gray-50">
          {/* Hero Banner */}
          <div className="bg-linear-to-r from-green-600 to-teal-600 text-white shadow-lg">
            <div className="max-w-7xl mx-auto px-3 sm:px-4 py-6 sm:py-8">
              <div className="flex flex-col sm:flex-row items-center justify-between gap-4">
                <div className="animate-fade-in text-center sm:text-left">
                  <h1 className="text-2xl sm:text-3xl md:text-4xl font-bold">
                    {useFallbackData ? 'Premium Shoe Collection' : 'Men\'s Shoes Collection'}
                  </h1>
                  <p className="mt-1 sm:mt-2 opacity-90 text-sm sm:text-base">
                    {filteredShoes.length} Premium Pairs Await
                    {error && (
                ...
                <div className="relative max-w-xl mx-auto">
                  <div className="relative">
                    ...
                    <input
                      ...
                      value={searchQuery}
                      onChange={(e) => setSearchQuery(e.target.value)}
                      className="w-full pl-10 sm:pl-12 pr-4 py-2 sm:py-3 bg-gray-50 border border-gray-200 rounded-full focus:outline-none focus:ring-2 focus:ring-green-500 focus:border-transparent text-sm sm:text-base"
                    />
                    {searchQuery && (
                      <button
                        onClick={() => setSearchQuery('')}
          ...
          <div className="max-w-7xl mx-auto px-3 sm:px-4 py-6 sm:py-8 md:py-12">
            ...
            <div className="mb-4 sm:mb-6 md:mb-8">
              <h2 className="text-lg sm:text-xl md:text-2xl lg:text-3xl font-bold text-gray-900">Premium Men's Shoes</h2>
              <p className="mt-1 sm:mt-2 text-gray-600 text-sm sm:text-base">
                Step up your style with premium men's footwear • Showing {filteredShoes.length} pair{filteredShoes.length !== 1 ? 's' : ''}
                {useFallbackData && (
                  <span className="ml-2 text-xs text-yellow-600 bg-yellow-50 px-2 py-1 rounded">
                    Using sample shoes only
                  </span>
                )}
              </p>
            </div>
            ...
            {filteredShoes.length > 0 ? (
              ...
              <div className="text-center py-12 sm:py-16 md:py-20">
                ...
                <button
                  onClick={clearAllFilters}
                  className="bg-linear-to-r from-green-600 to-teal-600 text-white px-6 sm:px-8 py-2.5 sm:py-3 rounded-full font-semibold hover:from-green-700 hover:to-teal-700 transition-all duration-300 transform hover:scale-105 shadow-lg text-sm sm:text-base"
                >
                  Explore All Shoes
                </button>
              </div>
          ...
          {/* Features Section */}
          <div className="bg-linear-to-r from-green-50 to-teal-50 py-8 sm:py-12 md:py-16 mt-8 sm:mt-12">
            <div className="max-w-7xl mx-auto px-3 sm:px-4">
              <h2 className="text-xl sm:text-2xl md:text-3xl font-bold text-center text-gray-900 mb-8 sm:mb-12">Why Choose Our Men's Shoes?</h2>
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-6 md:gap-8">
                <div className="bg-white p-4 sm:p-6 rounded-xl shadow-md">
                  <div className="flex items-center mb-3 sm:mb-4">
                    ...
                    <h3 className="font-semibold text-lg text-gray-900">Premium Materials</h3>
                  </div>
                  <p className="text-gray-600 text-sm sm:text-base">Crafted from premium leathers and durable materials for long-lasting wear</p>
                </div>
                <div className="bg-white p-4 sm:p-6 rounded-xl shadow-md">
                  <div className="flex items-center mb-3 sm:mb-4">
                    <div className="w-10 h-10 sm:w-12 sm:h-12 bg-green-100 rounded-full flex items-center justify-center mr-3">
                    ...
                    <h3 className="font-semibold text-lg text-gray-900">Perfect Fit Guarantee</h3>
                  </div>
                  <p className="text-gray-600 text-sm sm:text-base">Wide range of sizes with free exchanges to ensure perfect fit</p>
                </div>
                <div className="bg-white p-4 sm:p-6 rounded-xl shadow-md">
                  <div className="flex items-center mb-3 sm:mb-4">
                    <div className="w-10 h-10 sm:w-12 sm:h-12 bg-green-100 rounded-full flex items-center justify-center mr-3">
                    ...
                    <h3 className="font-semibold text-lg text-gray-900">Free Shipping & Returns</h3>
                  </div>
                  <p className="text-gray-600 text-sm sm:text-base">Free delivery and 45-day return policy on all shoe orders</p>
                </div>
              </div>
            </div>
          </div>
          ...
          <div className="bg-white py-8 sm:py-12">
            <div className="max-w-7xl mx-auto px-3 sm:px-4">
              <h2 className="text-xl sm:text-2xl md:text-3xl font-bold text-center text-gray-900 mb-6 sm:mb-8">Men's Shoe Size Guide</h2>
              <div className="overflow-x-auto">
                <table className="min-w-full bg-white border border-gray-200 rounded-lg">
                  <thead>
                    <tr className="bg-gray-50">
                      <th className="py-3 px-3 sm:px-4 text-left font-semibold text-gray-700 text-sm sm:text-base">US Size</th>
                      <th className="py-3 px-3 sm:px-4 text-left font-semibold text-gray-700 text-sm sm:text-base">EU Size</th>
                      <th className="py-3 px-3 sm:px-4 text-left font-semibold text-gray-700 text-sm sm:text-base">UK Size</th>
                      <th className="py-3 px-3 sm:px-4 text-left font-semibold text-gray-700 text-sm sm:text-base">Foot Length (cm)</th>
                    </tr>
                  </thead>
                  <tbody>
                    <tr className="border-t border-gray-200">
                      <td className="py-3 px-3 sm:px-4 text-sm sm:text-base">US 8</td>
                      <td className="py-3 px-3 sm:px-4 text-sm sm:text-base">41</td>
                      <td className="py-3 px-3 sm:px-4 text-sm sm:text-base">7</td>
                      <td className="py-3 px-3 sm:px-4 text-sm sm:text-base">25.4 cm</td>
                    </tr>
                    <tr className="border-t border-gray-200 bg-gray-50">
                      <td className="py-3 px-3 sm:px-4 text-sm sm:text-base">US 9</td>
                      <td className="py-3 px-3 sm:px-4 text-sm sm:text-base">42</td>
                      <td className="py-3 px-3 sm:px-4 text-sm sm:text-base">8</td>
                      <td className="py-3 px-3 sm:px-4 text-sm sm:text-base">26.0 cm</td>
                    </tr>
                    <tr className="border-t border-gray-200">
                      <td className="py-3 px-3 sm:px-4 text-sm sm:text-base">US 10</td>
                      <td className="py-3 px-3 sm:px-4 text-sm sm:text-base">43</td>
                      <td className="py-3 px-3 sm:px-4 text-sm sm:text-base">9</td>
                      <td className="py-3 px-3 sm:px-4 text-sm sm:text-base">26.7 cm</td>
                    </tr>
                    <tr className="border-t border-gray-200 bg-gray-50">
                      <td className="py-3 px-3 sm:px-4 text-sm sm:text-base">US 11</td>
                      <td className="py-3 px-3 sm:px-4 text-sm sm:text-base">44</td>
                      <td className="py-3 px-3 sm:px-4 text-sm sm:text-base">10</td>
                      <td className="py-3 px-3 sm:px-4 text-sm sm:text-base">27.3 cm</td>
                    </tr>
                    <tr className="border-t border-gray-200">
                      <td className="py-3 px-3 sm:px-4 text-sm sm:text-base">US 12</td>
                      <td className="py-3 px-3 sm:px-4 text-sm sm:text-base">45</td>
                      <td className="py-3 px-3 sm:px-4 text-sm sm:text-base">11</td>
                      <td className="py-3 px-3 sm:px-4 text-sm sm:text-base">28.0 cm</td>
                    </tr>
                  </tbody>
                </table>
              </div>
              <div className="mt-6 sm:mt-8 text-center text-gray-600">
                <p className="text-sm sm:text-base">Measure your foot from heel to longest toe for accurate sizing</p>
              </div>
            </div>
          </div>
        </div>
